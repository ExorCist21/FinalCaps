    ...
    <div class="max-w-7xl mx-auto mt-8">
        ...
        @if ($subscriptions->isEmpty())
            <div class="text-center text-gray-600 bg-pastel-gray-100 p-8 rounded-lg shadow-md">
                <p class="text-lg font-semibold">No subscriptions found.</p>
                <p class="mt-2 text-sm">Please subscribe a plan to get started.</p>
            </div>
        @else
            <!-- Sessions List -->
                ...
                @foreach ($subscriptions as $subscription)
                    <div class="bg-white border rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300 p-6">
                        ...
                        <p class="mt-3 text-sm text-gray-600">
                            <strong>Session Added:</strong> {{ $subscription->duration }} Day(s)
                        </p>
                        ...
                        @if ($subscription->status !== 'active')
                            <div class="flex justify-between items-center mt-4">
                                <form action="{{ url('/therapist/subscriptions/' . $subscription->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
